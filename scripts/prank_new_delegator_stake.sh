...
#      - `PRIVATE_KEY_NEW_DELEGATOR=0x...` (new delegator private key)
#   2. Prepare story client to localnet root folder
# The script performs:
#   1. Get validator info from PRIVATE_KEY
#   2. Get new delegator info from PRIVATE_KEY_NEW_DELEGATOR
...
if [[ -z "${PRIVATE_KEY_NEW_DELEGATOR:-}" ]]; then
  echo "Set PRIVATE_KEY_NEW_DELEGATOR in ${PRIVATE_KEY_FILE} (or export it) for the new delegator account." >&2
  exit 1
fi

PRIVATE_KEY_STRIPPED=${PRIVATE_KEY#0x}
PRIVATE_KEY_NEW_DELEGATOR_STRIPPED=${PRIVATE_KEY_NEW_DELEGATOR#0x}
...
if [[ ${#PRIVATE_KEY_NEW_DELEGATOR_STRIPPED} -ne 64 ]]; then
  echo "PRIVATE_KEY_NEW_DELEGATOR must be a 32-byte hex string (optionally 0x-prefixed)." >&2
  exit 1
fi
...
# Get new delegator info from PRIVATE_KEY_NEW_DELEGATOR
...
if [[ -z "$NEW_DEL_ADDRESS_BECH32" ]]; then
  echo "Failed to derive new delegator address from PRIVATE_KEY_NEW_DELEGATOR. Please set NEW_DEL_ADDRESS_BECH32 explicitly." >&2
  exit 1
fi

if [[ -z "$NEW_DEL_ADDRESS_EVM" ]]; then
  echo "Failed to derive new delegator EVM address from PRIVATE_KEY_NEW_DELEGATOR. Please set NEW_DEL_ADDRESS_EVM explicitly." >&2
  exit 1
fi
