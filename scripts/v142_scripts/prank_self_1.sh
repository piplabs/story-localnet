...
# The script performs: create validator with 110,000 IP -> stake 1,024 IP -> unstake 110,000 IP + 1 IP
# it will NOT panic in this case since the remaining delegation (110,000 + 1,024 - 110,001 = 1023) is less than min_delegation
#  so all delegated tokens will be undelegated.
...
MIN_STAKE=${MIN_STAKE:-1024000000000000000000}            # 1,024 IP (ether units)
...
echo
echo "=== Step 2: stake ${MIN_STAKE} ==="
run_validator_cmd stake \
...
echo
WITHDRAW_AMOUNT=$(python3 - <<PY
create = int("${CREATE_AMOUNT}")
dust = 10**18  # 1 IP
print(create + dust)
PY
)
echo "=== Step 3: unstake ${WITHDRAW_AMOUNT} ==="
run_validator_cmd unstake \
  --validator-pubkey "$VAL_CMP_PUBKEY" \
  --unstake "${WITHDRAW_AMOUNT}" \
  --delegation-id 0 \
  --chain-id "$CHAIN_ID" \
  --rpc "$EL_RPC" \
  --explorer "$EXPLORER_URL" \
